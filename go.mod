module github.com/vielendanke/go-micro-example

go 1.16

require (
	github.com/google/go-cmp v0.5.4 // indirect
	github.com/lib/pq v1.10.2
	github.com/unistack-org/micro-client-http/v3 v3.3.11
	github.com/unistack-org/micro-codec-json/v3 v3.2.5
	github.com/unistack-org/micro-config-consul/v3 v3.2.6
	github.com/unistack-org/micro-config-env/v3 v3.3.0
	github.com/unistack-org/micro-config-file/v3 v3.3.0
	github.com/unistack-org/micro-config-vault/v3 v3.2.9
	github.com/unistack-org/micro-proto v0.0.1
	github.com/unistack-org/micro-server-http/v3 v3.3.19
	github.com/unistack-org/micro/v3 v3.3.19
	golang.org/x/net v0.0.0-20210525063256-abc453219eb5 // indirect
	golang.org/x/xerrors v0.0.0-20200804184101-5ec99f83aff1 // indirect
	google.golang.org/protobuf v1.25.0
)
